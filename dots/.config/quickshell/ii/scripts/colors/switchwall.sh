...
NONINTERACTIVE="${SWITCHWALL_NONINTERACTIVE:-0}"
...
switch() {
        ...
        if is_video "$imgpath"; then
            ...
            if [ ${#missing_deps[@]} -gt 0 ]; then
                ...
                if [ "$NONINTERACTIVE" = "1" ]; then
                    echo "[switchwall] missing video deps, cannot proceed non-interactively" >&2
                    exit 1
                fi
                action=$(notify-send \
                    -a "Wallpaper switcher" \
    ...
    tmp_scss="$STATE_DIR/user/generated/material_colors.scss.tmp"
    if source "${ILLOGICAL_IMPULSE_VIRTUAL_ENV/#\~/$HOME}/bin/activate" && python3 "$SCRIPT_DIR/generate_colors_material.py" "${generate_colors_material_args[@]}" > "$tmp_scss"; then
        mv "$tmp_scss" "$STATE_DIR/user/generated/material_colors.scss"
    else
        rm -f "$tmp_scss"
        echo "[switchwall] color generation failed, keeping previous scss" >&2
        deactivate
        return 1
    fi
    deactivate
...
main() {
        ...
        case "$1" in
            ...
            --color)
                if [[ "$2" =~ ^#?[A-Fa-f0-9]{6}$ ]]; then
                    ...
                    if [ "$NONINTERACTIVE" = "1" ]; then
                        echo "[switchwall] --color interactive picker not available non-interactively" >&2
                        exit 1
                    fi
    ...
    if [[ -z "$imgpath" && -z "$color_flag" && -z "$noswitch_flag" ]]; then
        if [ "$NONINTERACTIVE" = "1" ]; then
            echo "[switchwall] no imgpath provided and kdialog not available non-interactively" >&2
            exit 1
        fi
